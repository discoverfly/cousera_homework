...
class Point < GeometryValue
  ...
  def intersectPoint other
    if real_close_point(x, y, other.x, other.y) then self else NoPoints.new end
  end
...
class Line < GeometryValue
  ...
  def intersectLine other
    if real_close(m, other.m)
      if real_close(b, other.b) then self else NoPoints.new end
    else
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(x1, x2)
      ...
      a, b = self, seg
      if y1 >= seg.y1 then a, b = seg, self end

      if real_close(a.y2, b.y1) then Point.new(a.x2, a.y2)
      elsif a.y2 < b.y1 then NoPoints.new
      elsif a.y2 > b.y2 then b
      else LineSegment.new(b.x1, b.y1, a.x2, a.y2) end
    else
      #the segments are on a (non-vertical) line
      a, b = self, seg
      if x1 >= seg.x1 then a, b = seg, self end
      
      if real_close(a.x2, b.x1) then Point(a.x2, a.y2)
      elsif a.x2 < b.x1 then NoPoints.new
      elsif a.x2 > b.x2 then b
      else LineSegment.new(b.x1, b.y1, a.x2, a.y2) end
    end
  end
end
